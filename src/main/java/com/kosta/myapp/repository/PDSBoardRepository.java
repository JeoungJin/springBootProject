...
public interface PDSBoardRepository 
      extends PagingAndSortingRepository<PDSBoard, Long> {
	// 1.기본제공되는 메서드만 가능 : findAll(), findAll(Sort)
	// 2.규칙에 맞는 함수정의가능
	// 3.JPQL : @Query이용해서 query직접사용가능
	@Query("select b.pid, b.pname, b.pwriter,f.fno, f.pdsfilename   from PDSBoard b left outer join b.files2 f "
			+ "  order by b.pid ")
