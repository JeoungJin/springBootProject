...
public interface WebBoardRepository extends PagingAndSortingRepository<WebBoard, Long>, 
   QuerydslPredicateExecutor<WebBoard> {
	...
	//1.추상메서드 : 정의는 있고 구현은 없다. 구현은 implements받은 class가 한다. 
	//3.static메서드 : interface를 구현한 모든class가 공유해서 사용하는 메서드이다. 재정의 불가능하다. 
	//2.default메서드 : interface를 구현한 모든class가 공유해서 사용하는 메서드이다. 재정의가능하다. 
	public default Predicate makePredicate(String type, String keyword) {
		...
		switch (type) {
		case "title": builder.and(board.title.like("%" + keyword + "%")); break; //and title like '%'?'%'
